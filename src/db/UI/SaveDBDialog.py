import tkinter as tk
from tkinter import ttk

from src.db.UI.GlobalUI import GlobalUI
from src.db.UI.Strings import Strings


class SaveDBDialog:
    def __init__(self, parent,
                 proceed_callback,
                 title=Strings.Current.DIALOG_ATTENTION,
                 proceed_text=Strings.Current.DIALOG_PROCEED,
                 cancel_text=Strings.Current.DIALOG_CANCEL,
                 width = 400,
                 height = 200,
                 top_pad_y=20,
                 close_before_callback=False):
        self.close_before_callback = close_before_callback
        self.proceed_callback = proceed_callback
        self.top = tk.Toplevel(parent)
        self.top.title(title)
        self.width = width
        self.height = height
        self.top.transient(parent)
        self.top.grab_set()
        self.db_name = GlobalUI.new_db_name
        self.top.resizable(False, False)
        self.center_window(parent)
        self.top.iconbitmap(GlobalUI.icon)
        message = Strings.Current.GENERATE_DB_MESSAGE.replace('{%s1}', GlobalUI.folder_path).replace('{%s2}', GlobalUI.new_db_name)
        self.msg_label = ttk.Label(self.top, text=message, font=("Consolas", 10), wraplength=width-20, justify="center")
        self.msg_label.pack(pady=(top_pad_y, 10))
        self.check_var = tk.Variable()
        chk = ttk.Checkbutton(self.top, text=Strings.Current.SAVE_DB_CHECK_BOX_TITLE, variable=self.check_var, command=self.on_check_box_click)
        chk.pack(pady=(8, 0))

        btn_frame = ttk.Frame(self.top)
        btn_frame.pack(side="bottom", pady=15)

        proceed_btn = ttk.Button(btn_frame, text=proceed_text, command=self.proceed, width=12)
        proceed_btn.pack(side="left", padx=8)

        cancel_btn = ttk.Button(btn_frame, text=cancel_text, command=self.cancel, width=12)
        cancel_btn.pack(side="left", padx=8)

        self.top.bind("<Return>", lambda e: self.proceed())
        self.top.bind("<Escape>", lambda e: self.cancel())

        self.top.focus_force()
        self.top.update_idletasks()

    def on_check_box_click(self):
        self.db_name = GlobalUI.new_db_name
        if str(self.check_var.get()) == '1':
            self.db_name = GlobalUI.db_name
        message = Strings.Current.GENERATE_DB_MESSAGE.replace('{%s1}', GlobalUI.folder_path).replace('{%s2}',
                                                                                                     self.db_name)
        self.msg_label.config(text=message)

    def center_window(self, parent):
        self.top.update_idletasks()
        x = parent.winfo_x() + (parent.winfo_width() // 2) - self.width // 2
        y = parent.winfo_y() + (parent.winfo_height() // 2) - self.height // 2
        self.top.geometry(f"{self.width}x{self.height}+{x}+{y}")

    def proceed(self):
        if self.close_before_callback:
            self.top.destroy()
        self.proceed_callback(self.db_name)
        if not self.close_before_callback:
            self.top.destroy()

    def cancel(self):
        self.top.destroy()